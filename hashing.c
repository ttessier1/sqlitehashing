...
)
{
    ...
    DebugMessage("Md4MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
...
)
{
    ...
    DebugMessage("Md5MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
...
{
    sqlite3* db = NULL;
    Sha1MacBlobContextPtr sha1MacBlobContext;
    ...
    DebugMessage("Sha1MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha1MacBlobContext = Sha1MacInitialize(fromHex, resultLength);
                        if (sha1MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha1MacUpdate(sha1MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha1MacFinalize(sha1MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha1MacBlobContext = Sha1MacInitialize(zKey, keyIn);
                    if (sha1MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha224MacBlobContextPtr sha224MacBlobContext;
    ...
    DebugMessage("Sha224MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha224MacBlobContext = Sha224MacInitialize(fromHex, resultLength);
                        if (sha224MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha224MacUpdate(sha224MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha224MacFinalize(sha224MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha224MacBlobContext = Sha224MacInitialize(zKey, keyIn);
                    if (sha224MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha256MacBlobContextPtr sha256MacBlobContext;
    ...
    DebugMessage("Sha256MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha256MacBlobContext = Sha256MacInitialize(fromHex, resultLength);
                        if (sha256MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha256MacUpdate(sha256MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha256MacFinalize(sha256MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha256MacBlobContext = Sha256MacInitialize(zKey, keyIn);
                    if (sha256MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha384MacBlobContextPtr sha384MacBlobContext;
    ...
    DebugMessage("Sha384MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha384MacBlobContext = Sha384MacInitialize(fromHex, resultLength);
                        if (sha384MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha384MacUpdate(sha384MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha384MacFinalize(sha384MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha384MacBlobContext = Sha384MacInitialize(zKey, keyIn);
                    if (sha384MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha512MacBlobContextPtr sha512MacBlobContext;
    ...
    DebugMessage("Sha512MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha512MacBlobContext = Sha512MacInitialize(fromHex, resultLength);
                        if (sha512MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha512MacUpdate(sha512MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha512MacFinalize(sha512MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha512MacBlobContext = Sha512MacInitialize(zKey, keyIn);
                    if (sha512MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha3224MacBlobContextPtr sha3224MacBlobContext;
    ...
    DebugMessage("Sha3224MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha3224MacBlobContext = Sha3224MacInitialize(fromHex, resultLength);
                        if (sha3224MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha3224MacUpdate(sha3224MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha3224MacFinalize(sha3224MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha3224MacBlobContext = Sha3224MacInitialize(zKey, keyIn);
                    if (sha3224MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha3256MacBlobContextPtr sha3256MacBlobContext;
    ...
    DebugMessage("Sha3256MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha3256MacBlobContext = Sha3256MacInitialize(fromHex, resultLength);
                        if (sha3256MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha3256MacUpdate(sha3256MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha3256MacFinalize(sha3256MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha3256MacBlobContext = Sha3256MacInitialize(zKey, keyIn);
                    if (sha3256MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha3384MacBlobContextPtr sha3384MacBlobContext;
    ...
    DebugMessage("Sha3384MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha3384MacBlobContext = Sha3384MacInitialize(fromHex, resultLength);
                        if (sha3384MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha3384MacUpdate(sha3384MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha3384MacFinalize(sha3384MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha3384MacBlobContext = Sha3384MacInitialize(zKey, keyIn);
                    if (sha3384MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sha3512MacBlobContextPtr sha3512MacBlobContext;
    ...
    DebugMessage("Sha3512MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sha3512MacBlobContext = Sha3512MacInitialize(fromHex, resultLength);
                        if (sha3512MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sha3512MacUpdate(sha3512MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sha3512MacFinalize(sha3512MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sha3512MacBlobContext = Sha3512MacInitialize(zKey, keyIn);
                    if (sha3512MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    RipeMD128MacBlobContextPtr ripemd128MacBlobContext;
    ...
    DebugMessage("RipeMD128MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        ripemd128MacBlobContext = RipeMD128MacInitialize(fromHex, resultLength);
                        if (ripemd128MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    RipeMD128MacUpdate(ripemd128MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = RipeMD128MacFinalize(ripemd128MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    ripemd128MacBlobContext = RipeMD128MacInitialize(zKey, keyIn);
                    if (ripemd128MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    RipeMD160MacBlobContextPtr ripemd160MacBlobContext;
    ...
    DebugMessage("RipeMD160MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        ripemd160MacBlobContext = RipeMD160MacInitialize(fromHex, resultLength);
                        if (ripemd160MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    RipeMD160MacUpdate(ripemd160MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = RipeMD160MacFinalize(ripemd160MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    ripemd160MacBlobContext = RipeMD160MacInitialize(zKey, keyIn);
                    if (ripemd160MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    RipeMD256MacBlobContextPtr ripemd256MacBlobContext;
    ...
    DebugMessage("RipeMD256MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        ripemd256MacBlobContext = RipeMD256MacInitialize(fromHex, resultLength);
                        if (ripemd256MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    RipeMD256MacUpdate(ripemd256MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = RipeMD256MacFinalize(ripemd256MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    ripemd256MacBlobContext = RipeMD256MacInitialize(zKey, keyIn);
                    if (ripemd256MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    RipeMD320MacBlobContextPtr ripemd320MacBlobContext;
    ...
    DebugMessage("RipeMD320MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        ripemd320MacBlobContext = RipeMD320MacInitialize(fromHex, resultLength);
                        if (ripemd320MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    RipeMD320MacUpdate(ripemd320MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = RipeMD320MacFinalize(ripemd320MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    ripemd320MacBlobContext = RipeMD320MacInitialize(zKey, keyIn);
                    if (ripemd320MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Blake2BMacBlobContextPtr blake2bMacBlobContext;
    ...
    DebugMessage("Blake2BMacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        blake2bMacBlobContext = Blake2BMacInitialize(fromHex, resultLength);
                        if (blake2bMacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Blake2BMacUpdate(blake2bMacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Blake2BMacFinalize(blake2bMacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    blake2bMacBlobContext = Blake2BMacInitialize(zKey, keyIn);
                    if (blake2bMacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Blake2SMacBlobContextPtr blake2sMacBlobContext;
    ...
    DebugMessage("Blake2SMacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        blake2sMacBlobContext = Blake2SMacInitialize(fromHex, resultLength);
                        if (blake2sMacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Blake2SMacUpdate(blake2sMacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Blake2SMacFinalize(blake2sMacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    blake2sMacBlobContext = Blake2SMacInitialize(zKey, keyIn);
                    if (blake2sMacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    TigerMacBlobContextPtr tigerMacBlobContext;
    ...
    DebugMessage("TigerMacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        tigerMacBlobContext = TigerMacInitialize(fromHex, resultLength);
                        if (tigerMacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    TigerMacUpdate(tigerMacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = TigerMacFinalize(tigerMacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    tigerMacBlobContext = TigerMacInitialize(zKey, keyIn);
                    if (tigerMacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Shake128MacBlobContextPtr shake128MacBlobContext;
    ...
    DebugMessage("Shake128MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        shake128MacBlobContext = Shake128MacInitialize(fromHex, resultLength);
                        if (shake128MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Shake128MacUpdate(shake128MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Shake128MacFinalize(shake128MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    shake128MacBlobContext = Shake128MacInitialize(zKey, keyIn);
                    if (shake128MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Shake256MacBlobContextPtr shake256MacBlobContext;
    ...
    DebugMessage("Shake256MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        shake256MacBlobContext = Shake256MacInitialize(fromHex, resultLength);
                        if (shake256MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Shake256MacUpdate(shake256MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Shake256MacFinalize(shake256MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    shake256MacBlobContext = Shake256MacInitialize(zKey, keyIn);
                    if (shake256MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Siphash64MacBlobContextPtr siphash64MacBlobContext;
    ...
    DebugMessage("Siphash64MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        siphash64MacBlobContext = Siphash64MacInitialize(fromHex, resultLength);
                        if (siphash64MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Siphash64MacUpdate(siphash64MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Siphash64MacFinalize(siphash64MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    siphash64MacBlobContext = Siphash64MacInitialize(zKey, keyIn);
                    if (siphash64MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Siphash128MacBlobContextPtr siphash128MacBlobContext;
    ...
    DebugMessage("Siphash128MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        siphash128MacBlobContext = Siphash128MacInitialize(fromHex, resultLength);
                        if (siphash128MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Siphash128MacUpdate(siphash128MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Siphash128MacFinalize(siphash128MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    siphash128MacBlobContext = Siphash128MacInitialize(zKey, keyIn);
                    if (siphash128MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Lsh224MacBlobContextPtr lsh224MacBlobContext;
    ...
    DebugMessage("Lsh224MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        lsh224MacBlobContext = Lsh224MacInitialize(fromHex, resultLength);
                        if (lsh224MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Lsh224MacUpdate(lsh224MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Lsh224MacFinalize(lsh224MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    lsh224MacBlobContext = Lsh224MacInitialize(zKey, keyIn);
                    if (lsh224MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Lsh256MacBlobContextPtr lsh256MacBlobContext;
    ...
    DebugMessage("Lsh256MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        lsh256MacBlobContext = Lsh256MacInitialize(fromHex, resultLength);
                        if (lsh256MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Lsh256MacUpdate(lsh256MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Lsh256MacFinalize(lsh256MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    lsh256MacBlobContext = Lsh256MacInitialize(zKey, keyIn);
                    if (lsh256MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Lsh384MacBlobContextPtr lsh384MacBlobContext;
    ...
    DebugMessage("Lsh384MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        lsh384MacBlobContext = Lsh384MacInitialize(fromHex, resultLength);
                        if (lsh384MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Lsh384MacUpdate(lsh384MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Lsh384MacFinalize(lsh384MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    lsh384MacBlobContext = Lsh384MacInitialize(zKey, keyIn);
                    if (lsh384MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Lsh512MacBlobContextPtr lsh512MacBlobContext;
    ...
    DebugMessage("Lsh512MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        lsh512MacBlobContext = Lsh512MacInitialize(fromHex, resultLength);
                        if (lsh512MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Lsh512MacUpdate(lsh512MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Lsh512MacFinalize(lsh512MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    lsh512MacBlobContext = Lsh512MacInitialize(zKey, keyIn);
                    if (lsh512MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    Sm3MacBlobContextPtr sm3MacBlobContext;
    ...
    DebugMessage("Sm3MacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        sm3MacBlobContext = Sm3MacInitialize(fromHex, resultLength);
                        if (sm3MacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    Sm3MacUpdate(sm3MacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = Sm3MacFinalize(sm3MacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    sm3MacBlobContext = Sm3MacInitialize(zKey, keyIn);
                    if (sm3MacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
...
{
    sqlite3* db = NULL;
    WhirlpoolMacBlobContextPtr whirlpoolMacBlobContext;
    ...
    DebugMessage("WhirlpoolMacBlob Called\r\n");
    if (argc != 6)
    {
        DebugMessage("Test\r\n");
        sqlite3_result_error(context, "Invalid arguments", strlength("Invalid arguments"));
        ...
        if (rc == SQLITE_OK)
        {
            ...
            if (buffer != NULL)
            {
                if (sqlite3_value_int(argv[5]) == 1)
                {
                    ...
                    if (fromHex)
                    {
                        whirlpoolMacBlobContext = WhirlpoolMacInitialize(fromHex, resultLength);
                        if (whirlpoolMacBlobContext != NULL)
                        {
                            while (rc == SQLITE_OK && remainingSize > 0)
                            {
                                length = MIN(buffer_size, remainingSize);
                                ...
                                if (rc == SQLITE_OK)
                                {
                                    WhirlpoolMacUpdate(whirlpoolMacBlobContext, buffer, length);
                                    offset += length;
                                    remainingSize -= length;
                                }
                            }
                            if (rc == SQLITE_OK)
                            {
                                result = WhirlpoolMacFinalize(whirlpoolMacBlobContext);
                                if (result != NULL)
                                {
                                    nIn = strlength(result);
                                    zOut = zToFree = (unsigned char*)sqlite3_malloc64(nIn + 1);
                ...
                {
                    // dont do conversion from hex
                    whirlpoolMacBlobContext = WhirlpoolMacInitialize(zKey, keyIn);
                    if (whirlpoolMacBlobContext != NULL)
                    {
                        while (rc == SQLITE_OK && remainingSize > 0)
                        {
